...
def test_env():
    ...
    api_key = os.environ.get(
        'BLUELM_API_KEY',
        'sk-xuanji-2026707755-cGZFSXJTVmNkT0hwV0J2Vw=='
    )
    has_custom_key = 'BLUELM_API_KEY' in os.environ
    print(f"  API Key: {'✅ 已配置(.env)' if has_custom_key else '⚠️  使用内置比赛AppKey'}")
...
def test_cloud_api(api_key):
    ...
    try:
        ...
        print("  requests 库: ❌ 未安装，请运行 install.bat")
    ...
    print()
    print("  ⚠️  所有模型均不可用（网络问题或 AppKey 失效）")
...
def test_tts():
    ...
    try:
        ...
        print("  edge-tts 库: ⚠️  未安装（运行 install.bat 安装）")
...
def print_architecture():
  ...
  核心代码定位:
  - app.py L287-334: call_bluelm_cloud() — 云端API + 多模型轮询
  - app.py L337-402: generate_mock_reply() — 10类语义本地降级
